...
export async function convertShopeeLink(shopeeUrl, myId) {
  ...
  // Nếu là link rút gọn, resolve ra link đích trước khi lấy shopId/productId
  let finalUrl = shopeeUrl;
  if (SHORT_LINK_RE.test(parsed.hostname)) {
    try {
      finalUrl = await resolveShortLink(shopeeUrl);
    } catch {
      // Resolve lỗi → dùng nguyên link gốc, vẫn thử tiếp bên dưới
    }
  }
  ...
  const info = await fetchProductInfo(originLink);
