// lib/botData.js
//
// [CẬP NHẬT] donhang/vitien giờ đọc TRỰC TIẾP từ API của phuongthaovip
// (https://phuongthaovip.vercel.app/api/data/donhang | /vitien) — ĐÚNG CÁCH
...
// Vercel dễ lệch nhau; gọi thẳng API phuongthaovip đảm bảo hoan-vi-web luôn
// thấy ĐÚNG dữ liệu mà bot đang thấy, không cần đoán.
//
// Nếu gọi phuongthaovip lỗi/timeout → fallback về Redis riêng của
// hoan-vi-web (giữ nguyên hành vi cũ) để trang không sập hẳn.
//
// [SỬA] danhan_by_subid giờ ƯU TIÊN đọc qua GET /api/data/danhan của
// phuongthaovip (giống donhang/vitien), fallback về Upstash Redis riêng
// của hoan-vi-web nếu phuongthaovip lỗi/timeout. Trước đây danhan chỉ đọc
...
// trình khác (vd upload CSV bên phuongthaovip), khiến dashboard luôn hiện
// "Đã nhận: 0đ" dù bot log ghi thành công. Đọc qua API phuongthaovip loại
...
// biến môi trường PHUONGTHAOVIP_BASE_URL trên Vercel nếu domain đổi.
const PHUONGTHAOVIP_BASE_URL =
  process.env.PHUONGTHAOVIP_BASE_URL || "https://phuongthaovip.vercel.app";
...
// API phuongthaovip (GET /api/data/danhan) giống hệt donhang/vitien, thay
...
// thể bị một tiến trình khác (vd upload CSV bên phuongthaovip) ghi đè mất
// dữ liệu — gọi thẳng API phuongthaovip đảm bảo luôn lấy đúng nguồn dữ
// liệu mà bot đang dùng, không phụ thuộc vào việc 2 database Redis có
// đồng bộ đúng hay không. Nếu gọi phuongthaovip lỗi/timeout, code vẫn
...
/** GET https://phuongthaovip.vercel.app/api/data/<type> — giống hệt
...
async function fetchFromPhuongThaoVip(type) {
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), 8000);
  try {
    const res = await fetch(`${PHUONGTHAOVIP_BASE_URL}/api/data/${type}`, {
      headers: { Accept: "application/json" },
      cache: "no-store",
      signal: controller.signal,
    });
    if (!res.ok) {
      console.warn(`[fetchFromPhuongThaoVip] ${type} → HTTP ${res.status}`);
      return null;
    }
    ...
    console.warn(`[fetchFromPhuongThaoVip] ${type} lỗi:`, err?.message || err);
    return null;
  } finally {
 ...
 *
 * donhang_by_subid / vitien_by_subid: ưu tiên gọi API phuongthaovip (giống
...
export async function getBotData(key) {
  ...
  if (remoteType) {
    const remoteData = await fetchFromPhuongThaoVip(remoteType);
    if (remoteData) return remoteData;
    console.warn(
      `[getBotData] Không lấy được "${remoteType}" từ phuongthaovip, fallback Redis riêng...`
    );
  }
